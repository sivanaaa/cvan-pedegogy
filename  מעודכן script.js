...
async function submitQuestionnaire() {
  ...
  try {
    ...
    // Never fail silently: try the email client automatically, and also show
    // the raw answers as copyable text in case the mail client doesn't open
    // either (e.g. no configured mail app on this device).
    openMailtoFallback(messageText);
    errorBox.hidden = false;
    errorBox.innerHTML =
      'השליחה האוטומטית לא הצליחה הפעם. פתחנו עבורך את תוכנת המייל כדי לשלוח את התשובות ישירות — ' +
      'אם שום דבר לא נפתח, ' +
      '<a href="#" id="mailtoFallbackLink">אפשר ללחוץ כאן לנסות שוב</a>, ' +
